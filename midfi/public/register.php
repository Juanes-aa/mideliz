...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($nombre) || empty($apellido) || empty($email) || empty($password)) {
        ...
        try {
            ...
            $mensaje = "❌ Error: " . $e->getMessage(); // Mostrar el error real para depuración
            $tipo_mensaje = "error";
        }
    }
}
...
<body class="flex flex-col min-h-screen font-poppins bg-gradient-to-br from-red-50 via-white to-red-100">
      ...
      <div class="registro-card bg-white rounded-2xl shadow-2xl p-6 sm:p-8 lg:p-10">
        ...
        <div class="mt-6 sm:mt-8 text-center">
          <p class="text-gray-600 text-xs sm:text-sm">
            ¿Ya tienes cuenta? 
            <a href="login.php" class="text-red-600 font-semibold hover:text-red-700 hover:underline transition">
              Inicia sesión aquí
            </a>
          </p>
        </div>
  ...
  <script>
    window.addEventListener("load", () => {
      ...
      // Validación en tiempo real de contraseñas (sin animaciones)
